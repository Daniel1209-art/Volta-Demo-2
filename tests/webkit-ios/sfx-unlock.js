...
(async () => {
  ...
  if (bet.silence !== 0) fail('до эффекта на шине уже есть сигнал — измерение недостоверно');
  if (bet.sound < 0.05) fail(`эффект не дал звука (пик ${bet.sound}) — граф немой`);
  else console.log(`OK: после ОДНОГО тапа по логотипу эффект звучит, пик ${bet.sound}`);
